...
class Property:
    ...
    def __init__(self, db, propertyId):
        ...
        self.numPhotos = record["num_photos"]
        self.errorOccured = False
    ...
    def updateStatus(self):
        ...
        try:
            ...
            for node in invalidPropertyNodes:
                if "We are sorry but" in node.text or "This property has been removed" in node.text:
                    self.removed = True
                    break
    ...
    def getAdditionalMetadata(self):

        print "Property", self.propertyId

        if (self.numPhotos != None):
            return
        ...
        try:
            ...
            numPhotos = None
            photoCounters = soup.find_all("span", {"class":"gallery-main-status"})
            for node in photoCounters:
                if node != None:
                    numPhotos = int(node.text.split(" ")[2])
                    break

            if (numPhotos == None):
                pass

            if numPhotos != self.numPhotos:

                self.latestTimestamp = datetime.today().date()

                sql = "update property set num_photos = '{0}' where id = {1};"
                sql = sql.format(numPhotos, self.propertyId)
